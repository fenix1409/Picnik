...
const SinlgeProduct = () => {
  ...
  const count = useSelector((state: RootState) => state.product.counts[productid] || 1)
  ...
  return (
    <div className="mt-[13px] px-[100px] pb-[151px]">
      ...
      <div className="flex items-center gap-[130px] mb-[120px]">
        ...
        <div className="w-[612px]">
          ...
          <div className="flex items-center flex-col space-y-[18.5px] mb-[43px]">
            {descriptionsData.map(item => (
              <h2 key={item.id} className="text-[16px] leading-[24px] font-semibold">
                {item.title} <span className="font-medium text-[#727272]">{item.text}</span>
              </h2>
            ))}
          </div>
          <div className="flex items-center gap-[19px]">
            <div className="flex items-center gap-[49.5px] bg-[#F0F0F0] w-[193px] rounded-[62px] py-[14px] px-[20px]">
              <button onClick={() => dispatch(decrement(productid))} className="inline-block cursor-pointer"><Minus /></button>
              <span className="text-[16px] leading-[100%] font-medium">{count}</span>
              <button onClick={() => dispatch(increment(productid))} className="inline-block cursor-pointer"><Plus /></button>
            </div>
            <button className="w-[400px] py-[15px] px-[157px] text-[16px] leading-[100%] font-medium rounded-[62px] bg-[#245D30] text-white">
              Add to Cart
            </button>
          </div>
        </div>
      ...
      <div className="mx-auto text-center">
        ...
        <video controls width="100%" height="649px" className="rounded-[20px]">
          <source src={`${IMAGE_API}/${data.video_src}`} type="video/mp4" />
        </video>
      </div>
    </div>
  );
